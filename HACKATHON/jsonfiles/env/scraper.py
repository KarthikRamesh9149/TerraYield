...
CANDIDATE_STATE_UT_NAMES = [
    ...
    # Union Territories (names vary by dataset; we try common ones)
    "Andaman","Chandigarh","Dadra","Daman","Delhi",
    ,"Ladakh","Lakshadweep","Puducherry"
]

# Some repos use shorter/older names; add extra guesses here if you hit misses.
EXTRA_FILENAME_GUESSES = [
    "Orissa",                 # Odisha (older spelling)
    "Pondicherry",            # Puducherry (older)
    "Dadra and Nagar Haveli", # sometimes split/combined differently
    "Daman and Diu"
]
...
def build_url(template_url: str, state_name: str) -> str:
    # Replace the *filename stem* "Kerala" in ".../Kerala.geojson" with desired name.
    # Works as long as the template ends with "Kerala.geojson".
...
def process_one_state(url: str, state_name: str, outdir: str, state_prop: str,
    ...
    for feat in geo.get("features") or []:
        ...
        if write_per_district:
            district_path = os.path.join(outdir, f"{state_slug}__{district_slug}.json")
...
def main():
    ...
    ap.add_argument("--template-url", required=True,
                    help='Example: "https://raw.githubusercontent.com/.../Kerala.geojson"')
    ...
    if "Kerala.geojson" not in args.template_url:
        raise SystemExit('template-url must contain "Kerala.geojson" so it can be replaced.')
    ...
    for name in candidates:
        url = build_url(args.template_url, name)
        try:
